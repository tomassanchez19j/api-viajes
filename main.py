...
def insertar_venta_vuelo(user_data: esquema_ventas_vuelos):
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO ventas_de_vuelos(id_vuelo, id_comprador, fecha_compra, hora_compra, monto_total)
                VALUES (%s, %s,%s, %s, %s)
            """, (user_data.id_vuelo, user_data.id_comprador, user_data.fecha_compra, user_data.hora_compra, user_data.monto_total))
